...
class plagiarism_checker:
    ...
    def __init__(self):
        ...
        lbl_for_checking = tkinter.Label(self.root, text='Entry text to check',
    ...
    def calc_plagiarisedness(self):
        try:
            ...
            if amount_plagiarised_percentage > .5:  # If fits requirements for being plagiarised
                plagiarism_info_text = str(amount_plagiarised_percentage)\
                                       + '% Plagiarised'
                plagiarism_info_text.format(amount_plagiarised_percentage)
                lbl_plagiarism_info = tkinter.Label(self.plagiarised_window, text=plagiarism_info_text,
                                                    fg='red')
                lbl_plagiarism_info.config(font=('TkDefaultFont, 16'))
                lbl_plagiarism_info.place(x=32, y=39)
            else:  # If doesn't fit requirements for being plagiarised
